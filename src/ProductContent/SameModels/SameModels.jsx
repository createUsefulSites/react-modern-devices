...
import { fetchSameModels } from '../../redux/slices/modelSlice';
import ContentLoader from 'react-content-loader';

export default function SameModels({ currentId, category }) {
    const dispatch = useDispatch();
    const topModelsStatus = useSelector((state) => state.models.sameModelsStatus);

    useEffect(() => {
        dispatch(fetchSameModels(category));
    }, []);

    const sameModels = JSON.parse(localStorage.getItem('sameModels'));
    if (sameModels) {
        const currElement = sameModels.findIndex((el) => el.id === currentId);
        sameModels.splice(currElement, 1);
    }
